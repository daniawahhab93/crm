...
(function (window, document, $, undefined) {
  ...
  $(function () {
    ...
    $(".child_present").on("change", function () {
      $(".child_present").prop(
        $(".child_present:checked").length ? true : false
      );
    });
...
$(function () {
  ...
  $("body").on(
    "click",
    ".notifications a.n-top, .notification_link",
    function (e) {
      e.preventDefault();
      var $notLink = $(this);
...
function read_inline(id) {
  $.get(base_url + "admin/common/read_inline/" + id, function () {
    var notification = $("body").find(
      '.notification-li[data-notification-id="' + id + '"]'
    );
...
function fetch_notifications(callback) {
  $.get(
    base_url + "admin/common/get_notification",
    function (response) {
...
(function ($) {
  $.fn.appForm = function (options) {
    ...
    function maskModal($maskTarget) {
      ...
      $maskTarget.append(
        "<div class='modal-mask'><div class='circle-loader'></div></div>"
      );
      //check scrollbar
...
$("body").on(
  "change blur",
  ".apply_credit_invoices .amount-credit-field",
  function () {
...
$("body").on(
  "change blur",
  ".apply-invoice-credit .amount-credit-field",
  function () {
    var $creditApply = $("#credit_apply");
    var $amountInputs = $creditApply.find("input.amount-credit-field");
    var total = 0;
...
$(document).ready(function () {
  ...
  if ($searchBox.length) {
    $("#searchType option[value=" + searchType + "]").attr(
      "selected",
      "selected"
    );
  ...
  $searchBox.on("awesomplete-selectcomplete", function () {
    ...
    if (this.value) {
      ...
      if (searchFieldValue === "projects") {
        ...
        location =
          base_url + "admin/opportunities/opportunity_details/" + this.value;
        ...
        location =
          base_url +
          "admin/invoice/manage_invoice/invoice_details/" +
          this.value;
      } else if (searchFieldValue === "credit_note") {
        location =
          base_url +
          "admin/credit_note/index/credit_note_details/" +
          this.value;
      } else if (searchFieldValue === "estimates") {
        location =
          base_url + "admin/estimates/create/estimates_details/" + this.value;
      } else if (searchFieldValue === "tickets") {
        location =
          base_url + "admin/tickets/index/tickets_details/" + this.value;
      }
...
function handle_errors(error) {
  switch (error.code) {
    case error.PERMISSION_DENIED:
      alert("To mark attendence, you must allow location access");
      location.reload();
      break;
