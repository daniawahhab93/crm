 ...
 * @author NaYeM
 */
class Office_items_Model extends MY_Model
{
    ...
    public function make_all_office_items($project_id, $office_items_name = null, $tasks = null, $expense = null, $json = null)
    {
        $all_office_items = array();
        ...
        if (!empty($office_items_name)) {
            if ($office_items_name == 'single_line') {
                $office_items['office_items_id'] = 0;
                $office_items['saved_office_items_id'] = 'msp_' . $project_id;
                $office_items['item_name'] = $project_info->project_name;
                    ...
                    foreach ($tasks as $task_id) {
                        ...
                        if (!empty($all_tasks)) {
                            foreach ($all_tasks as $task) {
                                $t_seconds = $this->office_items_model->task_spent_time_by_id($task->task_id);
                                $t_init = $t_seconds;
                ...
                $office_items['item_desc'] = $description;
                $office_items['quantity'] = 1;
                $office_items['unit'] = null;
                $office_items['unit_cost'] = $project_cost;
                $office_items['rate'] = $project_cost;
                $all_office_items[] = (object)$office_items;
            }

            $total_timer = $this->db->where(array('project_id' => $project_info->project_id))->get('tbl_tasks_timer')->result();
            if ($office_items_name == 'project_timer' || $office_items_name == 'all_timesheet_individually') {
                if (!empty($total_timer)) {
                    foreach ($total_timer as $t_key => $v_timer) {
                        ...
                        if (!empty($p_timer_info)) {
                            ...
                            $office_items['office_items_id'] = 0;
                            $office_items['saved_office_items_id'] = 'mti' . $t_key;
                            $office_items['item_name'] = $project_info->project_name . ' ' . lang('by') . ' - ' . $user_info->username;
                            ...
                            $office_items['item_desc'] = $description;
                            $office_items['quantity'] = $qty;
                            $office_items['unit'] = null;
                            $office_items['unit_cost'] = $project_info->hourly_rate;
                            $office_items['rate'] = $project_info->hourly_rate;
                            $all_office_items[] = (object)$office_items;
                        }
                    }
                }
            }
            if ($office_items_name == 'all_timesheet_individually') {
                if (!empty($tasks)) {
                    foreach ($tasks as $task_id) {
                        ...
                        if (!empty($total_tasks_timer)) {
                            foreach ($total_tasks_timer as $t_key => $v_task_timer) {
                                ...
                                $office_items['office_items_id'] = 0;
                                $office_items['saved_office_items_id'] = 'mts' . $t_key;
                                $office_items['item_name'] = $project_info->project_name . ' - ' . $tasks_timer_info->task_name;
                ...
                foreach ($tasks as $task_id) {
                    ...
                    if (!empty($all_tasks)) {
                        foreach ($all_tasks as $key => $task) {
                            $total_seconds = $this->office_items_model->task_spent_time_by_id($task->task_id);
                            $init = $total_seconds;
                            ...
                            $qty = round($init / 3600, 2);
                            $office_items['office_items_id'] = 0;
                            $office_items['saved_office_items_id'] = 'mts' . $key;

                            $office_items['item_name'] = $project_info->project_name . ' - ' . $task->task_name;
                            ...
                            $office_items['quantity'] = $qty;
                            $office_items['unit'] = null;
                            $office_items['unit_cost'] = $task->hourly_rate;
                            $office_items['rate'] = $task->hourly_rate;
                            $office_items['task_id'] = $task_id;
        ...
        if (!empty($expense)) {
            foreach ($expense as $key => $expense_id) {
                ...
                $office_items['saved_office_items_id'] = 'mex' . $key;
                $office_items['office_items_id'] = 0;
                $office_items['item_name'] = '[ ' . lang('expense') . ' ] - ' . $category . ' - ' . $expense_info->name;;
                ...
                $office_items['quantity'] = 1;
                $office_items['unit'] = null;
                $office_items['unit_cost'] = $expense_info->amount;
                $office_items['rate'] = $expense_info->amount;
        ...
        if (!empty($json)) {
            return json_encode($all_office_items);
        }

        return $all_office_items;
    }
    ...
    {
        if (!empty($group_id)) {
            $result = get_result('tbl_saved_office_items', array('customer_group_id' => $group_id));
        } else {
            $result = get_result('tbl_saved_office_items');
        }
        return $result;
    }
