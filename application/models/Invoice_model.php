...
class Invoice_Model extends MY_Model
{
    ...
    public function total_sales($filter = null)
    {
        ...
        if ($filter && $filter != 'all') {
            $this->db->join('tbl_invoices', 'tbl_invoices.invoices_id= tbl_payments.invoices_id');
    ...
    public function paid_by_date_and_employee($year, $month = null, $filter = null)
    {
        $total = 0;
        $payments = [];
        if ($filter && $filter != 'all') {
    ...
    public function get_sales_report_details($data = null, $range = null)
    {

        if (!empty($data) && ($data['client_id'] != 'all'
                || $data['warehouse_id'] != 'all'
                || $data['saved_items_id'] != 'all'
                || $data['customer_group_id'] != 'all'
                || $data['user_id'] != 'all')
        ) {
            $this->db->join('tbl_warehouse', ' tbl_warehouse.warehouse_id =tbl_invoices.warehouse_id')
                ->join('tbl_items', 'tbl_items.invoices_id=tbl_invoices.invoices_id')
                ->join('tbl_saved_items', 'tbl_saved_items.saved_items_id =tbl_items.saved_items_id')
                ->join('tbl_customer_group', 'tbl_customer_group.customer_group_id=tbl_saved_items.customer_group_id')
                ->join('tbl_users', 'tbl_users.user_id=tbl_invoices.user_id');

            if ($data['client_id'] != 'all')
                $this->db->where('tbl_invoices.client_id', $data['client_id']);

            if ($data['warehouse_id'] != 'all')
                $this->db->where('tbl_invoices.warehouse_id', $data['warehouse_id']);

            if ($data['saved_items_id'] != 'all')
                $this->db->where('tbl_items.saved_items_id', $data['saved_items_id']);

            if ($data['customer_group_id'] != 'all')
                $this->db->where('tbl_saved_items.customer_group_id', $data['customer_group_id']);

            if ($data['user_id'] != 'all')
                $this->db->where('tbl_invoices.user_id', $data['user_id']);


            $all_data = $this->db->get('tbl_invoices')->result();
        } else {
            $all_data = $this->db->join('tbl_warehouse', ' tbl_warehouse.warehouse_id =tbl_invoices.warehouse_id')
                ->join('tbl_items', 'tbl_items.invoices_id=tbl_invoices.invoices_id')
                ->join('tbl_saved_items', 'tbl_saved_items.saved_items_id =tbl_items.saved_items_id')
                ->join('tbl_customer_group', 'tbl_customer_group.customer_group_id=tbl_saved_items.customer_group_id')
                ->join('tbl_users', 'tbl_users.user_id=tbl_invoices.user_id')
                ->get('tbl_invoices')->result();
        }
    ...
    public function get_sales_profit_report($data = null, $range = null)
    {
        if (!empty($data) && ($data['warehouse_id'] != 'all'  || $data['saved_items_id'] != 'all' )) {
            $this->db->join('tbl_invoices', 'tbl_invoices.invoices_id=tbl_items.invoices_id')
                ->join('tbl_warehouse', ' tbl_warehouse.warehouse_id =tbl_invoices.warehouse_id')
                ->join('tbl_saved_items', 'tbl_saved_items.saved_items_id =tbl_items.saved_items_id');
...
//                $this->db->where('tbl_invoices.client_id', $data['client_id']);
            ...
            if ($data['warehouse_id'] != 'all')
                $this->db->where('tbl_warehouse.warehouse_id', $data['warehouse_id']);
...
//                $this->db->where('tbl_invoices.user_id', $data['user_id']);
            ...
            $all_data = $this->db->get('tbl_items')->result();
        } else {

            $all_data = $this->db->join('tbl_invoices', 'tbl_invoices.invoices_id=tbl_items.invoices_id')
                ->join('tbl_warehouse', ' tbl_warehouse.warehouse_id =tbl_invoices.warehouse_id')
                ->join('tbl_saved_items', 'tbl_saved_items.saved_items_id =tbl_items.saved_items_id')
                ->get('tbl_items')->result();
    ...
    public function get_last_sale_price_item($id, $client_id)
    {
        $this->db->select('tbl_items.unit_cost as unit_cost');
        $this->db->join('tbl_invoices', 'tbl_invoices.invoices_id =tbl_items.invoices_id');
    ...
    public function get_lowest_sale_price_item($id, $client_id)
    {
        $this->db->select('tbl_items.unit_cost as unit_cost');
        $this->db->join('tbl_invoices', 'tbl_invoices.invoices_id =tbl_items.invoices_id');
        $this->db->where('client_id', $client_id);
