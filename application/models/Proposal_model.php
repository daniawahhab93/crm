...
class Proposal_Model extends MY_Model
{
    ...
    public function get_proposals_items_report($filterBy = null, $range = null)
    {
        $proposals_items =   $this->db->join('tbl_proposals', 'tbl_proposals.proposals_id=tbl_proposals_items.proposals_id')
            ->where('convert','No' )
            ->where('due_date <',date('Y-m-d'))
            ->get('tbl_proposals_items')->result();

        $proposals_items = array_reverse($proposals_items);

        if (!empty($proposals_items)) {
            $proposal_info = array();
            if (!empty($range[0])) {
                foreach ($proposals_items as $v_proposal) {
                    if ($v_proposal->date_saved >= $range[0] && $v_proposal->date_saved <= $range[1]) {
                        array_push($proposal_info, $v_proposal);
                    }
                }
                return $proposal_info;
            } else {
                return $proposals_items;
            }
        } else {
            return array();
        }
