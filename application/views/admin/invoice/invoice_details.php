...
<div class="row mb">
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu animated zoomIn">
                <?php if ($this->invoice_model->get_invoice_cost($invoice_info->invoices_id) > 0) { ?>
                    ...
                    <li>
                        <a
                                href="<?= base_url() ?>admin/invoice/manage_invoice/invoice_history/<?= $invoice_info->invoices_id ?>"><?= lang('invoice_history') ?></a>
                    </li>
                <?php } ?>
                ...
                if (!empty($can_edit) && !empty($edited)) { ?>
                    ...
                    <li>
                        <a
                                href="<?= base_url() ?>admin/invoice/createinvoice/create_invoice/<?= $invoice_info->invoices_id ?>"><?= lang('edit_invoice') ?></a>
                    </li>
                <?php } ?>
            </ul>
        </div>
    ...
    <div class="col-sm-2 pull-right">
        <a href="<?= base_url() ?>admin/invoice/send_invoice_email/<?= $invoice_info->invoices_id . '/' . true ?>"
            ...
            <div class="col-lg-4 col-xs-12 ">
                <div class="pull-right pr-lg">
                    ...
                    <?php if (!empty($text)) { ?>
                        <br>
                        <p
                                style="padding: 15px;margin-bottom: 20px;border: 1px solid transparent;border-radius: 4px;;background: color: #8a6d3b;background-color: #fcf8e3;border-color: #faebcc;">
                            <?= $text ?></p>
                    <?php } ?>
                    <?php $show_custom_fields = custom_form_label(9, $invoice_info->invoices_id);
                    if (!empty($show_custom_fields)) {
...
<ul class="pagination" style="float: right">
    ...
    $previous_payment = $this->db->where('invoices_id', ($invoice_info->invoices_id - 1))->get('tbl_invoices')->row();
    if ($previous_payment)
        $disable_previous_payment = '';

    $next_payment = $this->db->where('invoices_id', ($invoice_info->invoices_id + 1))->get('tbl_invoices')->row();
    if ($next_payment)
        $disable_next_payment = '';
    ?>
    <li class="paginate_button <?php echo $disable_previous_payment ?>">
        <a class="text-info"
           href="<?php echo base_url() . 'admin/invoice/manage_invoice/invoice_details/' . ($invoice_info->invoices_id - 1) ?>"> <?php echo lang('previous_payment') ?> </a>
    </li>
    <li class="paginate_button <?php echo $disable_next_payment ?> ">
        <a class="text-info"
           href="<?php echo base_url() . 'admin/invoice/manage_invoice/invoice_details/' . ($invoice_info->invoices_id + 1) ?>"> <?php echo lang('next_payment') ?> </a>
    </li>
</ul>
