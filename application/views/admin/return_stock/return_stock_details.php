...
<div class="row mb">
    <div class="col-sm-10">
        ...
        <a class="btn btn-xs btn-danger <?= $disable ?>" data-toggle="tooltip" data-placement="top" href="<?= $p_url ?>"
            title="<?= lang('received') . ' ' . lang('return_stock') ?>"><i class="fa fa-credit-card"></i>
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu animated zoomIn">
                ...
                <li>
                    <a
                        href="<?= base_url() ?>admin/return_stock/create_returnstock/<?= $return_stock_info->return_stock_id ?>"><?= lang('edit') . ' ' . lang('return_stock') ?></a>
                </li>
                <?php } ?>
            </ul>
        </div>
    ...
    <div class="col-sm-2 pull-right">
        ...
        <a onclick="print_return_stock('print_return_stock')" href="#" data-toggle="tooltip" data-placement="top"
            title="" data-original-title="Print" class="mr-sm btn btn-xs btn-danger pull-right">
        ...
        <a href="<?= base_url() ?>admin/return_stock/create_returnstock/<?= $return_stock_info->return_stock_id ?>"
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-xs-6 ">
                <div class="pull-right pr-lg">
                    ...
                    <br><?= lang('sales_agent') ?>: <?php echo fullname($return_stock_info->user_id); ?>
                    <?php }
                    if ($payment_status == ('fully_paid')) {
                        $label = "success";
...
<div class="panel panel-custom">
    <div class="panel-heading">
        <div class="panel-title"> <?= lang('payment_details') ?></div>
    </div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <a
                            href="<?= base_url() ?>admin/return_stock/payments_details/<?= $v_payments_info->payments_id ?>">
                            <?= $v_payments_info->trans_id; ?></a>
                    </td>
                    <td>
                        <a
                            href="<?= base_url() ?>admin/return_stock/payments_details/<?= $v_payments_info->payments_id ?>"><?= display_date($v_payments_info->payment_date); ?></a>
                    </td>
...
<ul class="pagination" style="float: right">
    ...
    $previous_payment=$this->db->where('return_stock_id', ($return_stock_info->return_stock_id-1))->get('tbl_return_stock')->row();
    if($previous_payment)
        $disable_previous_payment='';

    $next_payment=$this->db->where('return_stock_id', ($return_stock_info->return_stock_id+1))->get('tbl_return_stock')->row();
    if($next_payment)
        $disable_next_payment='';
    ?>
    <li class="paginate_button <?php echo $disable_previous_payment ?>">
        <a class="text-info" href="<?php echo base_url() . 'admin/return_stock/return_stock_details/' . ($return_stock_info->return_stock_id-1) ?>" > <?php echo lang('previous_payment')?> </a>
    </li>
    <li class="paginate_button <?php echo $disable_next_payment ?> "  >
        <a class="text-info" href="<?php echo base_url() . 'admin/return_stock/return_stock_details/' . ($return_stock_info->return_stock_id+1) ?>" > <?php echo lang('next_payment')?> </a>
    </li>
</ul>
