            ...
            <form role="form" data-parsley-validate="" novalidate="" id="userform" enctype="multipart/form-data"
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label"><strong><?= lang('password') ?> </strong><span
                            class="text-danger">*</span></label>
                    <div class="col-sm-5">
                        <input type="password" id="new_password" placeholder="<?= lang('password') ?>" name="password"
                            class="input-sm form-control" required>
                    </div>
                </div>
                <div class="form-group">
                ...
                <?php } ?>
                <div class="form-group">
                    <label class="col-sm-3 control-label"><strong><?= lang('email') ?> </strong></label>
                    <div class="col-sm-5">
...
//                                foreach ($locales as $loc) :
                                ?>
<!--                            <option lang="--><?php //echo $loc->code ?><!--" value="--><?php //echo $loc->locale ?><!--" -->
                                <?php
//                                                                                                    if (!empty($profile_info)) {
...
<!--                                <img src="http://placehold.it/350x260" alt="Please Connect Your Internet">-->
                ...
                <div class="form-group">
                    <label class="col-lg-3 control-label"><span><?= lang('warehouse') ?></span></label>
                    <div class="col-lg-4">
                        <select name='warehouses_ids[]' class="form-control selectpicker" data-live-search="true"
                                multiple>
                            <?php
                            $all_warehouses = get_result('tbl_warehouse');
                            foreach ($all_warehouses as $a_w) {
                                ?>
                                <?php $selected = '';
                                if ($profile_info->warehouses_ids != null)
                                    $warehouses_idssArray=json_decode($profile_info->warehouses_ids,true);
...
//                    $this->load->view('admin/items/warehouselist', $wData)
                    ...
                    <div class="col-sm-6 row">
                        <div id="client_permission" class="panel panel-custom ">
                            ...
                            <div class="form-horizontal">
                                ...
                                <?php
                                    $all_client_menu = $this->db->where('parent', 0)->where('appear', 1)->order_by('sort')->get('tbl_client_menu')->result();
                                    if (!empty($login_info)) {
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-6 checkbox">
                                        <input class="client_permission" data-toggle="toggle"
                                            name="<?= $v_menu->label ?>" value="<?= $v_menu->menu_id ?>"
                                            <?php
                                                                                                                                                                    if (!empty($user_menu)) {
                ...
                <div class="form-group" id="ratio_on_sale">
                    <label class="col-sm-3 control-label"><strong><?= lang('ratio_on_sale') ?> </strong></label>
                    <div class="col-sm-5">
                        <input type="text" data-parsley-type="number" min="0" max="100"  class="input-sm form-control" value="<?php
                        if (!empty($profile_info)) {
                            echo $profile_info->ratio_on_sale;
                        }else echo 0; ?>" name="ratio_on_sale">
                    </div>
                </div>
                <?php
...
$(document).ready(function() {
    ...
    $('#user_type').on('change', function() {
        if (this.value == '3' || this.value == '1') {
            ...
            $("#ratio_on_sale").show();
            $(".ratio_on_sale").removeAttr('disabled');
            ...
            $("#ratio_on_sale").hide();
            $(".ratio_on_sale").attr('disabled', 'disabled');
