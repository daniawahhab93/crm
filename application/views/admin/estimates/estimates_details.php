...
<div class="row mb">
    <div class="col-sm-12 mb">
        ...
        <div class="col-sm-10">
            <input style="width: 100%" class="form-control" value="<?= base_url() ?>frontend/estimates/<?= url_encode($estimates_info->estimates_id); ?>" type="text" id="foo" />
        </div>
    </div>
    ...
    <div class="col-sm-8">
        ...
        <?php if (!empty($can_edit) && !empty($edited)) { ?>
            ...
            <a data-toggle="modal" data-target="#myModal_lg" href="<?= base_url() ?>admin/estimates/insert_items/<?= $estimates_info->estimates_id ?>" title="<?= lang('item_quick_add') ?>" class="btn btn-xs btn-primary">
                <i class="fa fa-pencil text-white"></i> <?= lang('add_items') ?></a>

            <?php if ($estimates_info->show_client == 'Yes') { ?>
                <a class="btn btn-xs btn-success" href="<?= base_url() ?>admin/estimates/change_status/hide/<?= $estimates_info->estimates_id ?>" title="<?= lang('hide_to_client') ?>"><i class="fa fa-eye-slash"></i> <?= lang('hide_to_client') ?>
                </a><?php } else { ?>
                <a class="btn btn-xs btn-warning" href="<?= base_url() ?>admin/estimates/change_status/show/<?= $estimates_info->estimates_id ?>" title="<?= lang('show_to_client') ?>"><i class="fa fa-eye"></i> <?= lang('show_to_client') ?>
                </a><?php } ?>

            <a data-toggle="modal" data-target="#myModal_large" data-original-title="<?= lang('convert_to_invoice') ?>" data-toggle="tooltip" data-placement="top" class="btn btn-xs btn-purple <?php
                                                                                                                                                                                                if ($estimates_info->invoiced == 'Yes' or $estimates_info->client_id == '0') {
                                                                                                                                                                                                    echo "disabled";
                                                                                                                                                                                                }
                                                                                                                                                                                                ?>" href="<?= base_url() ?>admin/estimates/convert_to_invoice/<?= $estimates_info->estimates_id ?>" title="<?= lang('convert_to_invoice') ?>">
                <?= lang('convert_to_invoice') ?></a>
            <span data-toggle="tooltip" data-placement="top" title="<?= lang('clone') . ' ' . lang('estimate') ?>">
                <a data-toggle="modal" data-target="#myModal" title="<?= lang('clone') . ' ' . lang('estimate') ?>" href="<?= base_url() ?>admin/estimates/clone_estimate/<?= $estimates_info->estimates_id ?>" class="btn btn-xs btn-green">
        ...
        <?php if (!empty($can_edit) && !empty($edited)) { ?>
            <div class="btn-group">
                ...
                <ul class="dropdown-menu animated zoomIn">
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/create/email_estimates/<?= $estimates_info->estimates_id ?>" data-toggle="ajaxModal"><?= lang('email_estimate') ?></a>
                    </li>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/create/estimates_history/<?= $estimates_info->estimates_id ?>"><?= lang('estimate_history') ?></a>
                    </li>
                    ...
                    <?php if ($estimates_info->status == 'expired' || $estimates_info->status == 'sent' || $estimates_info->status == 'cancelled' || $estimates_info->status == 'draft') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/pending/<?= $estimates_info->estimates_id ?>" title="<?= lang('mark_as_pending') ?>"><?= lang('mark_as_pending') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status == 'draft') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/draft/<?= $estimates_info->estimates_id ?>" title="<?= lang('unmark_as_draft') ?>"><?= lang('mark_as_draft') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status != 'sent' || $estimates_info->status == 'expired') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/sent/<?= $estimates_info->estimates_id ?>" title="<?= lang('mark_as_sent') ?>"><?= lang('mark_as_sent') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status == 'pending' || $estimates_info->status == 'sent') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/expired/<?= $estimates_info->estimates_id ?>" title="<?= lang('mark_as_sent') ?>"><?= lang('mark_as_expired') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status != 'cancelled') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/cancelled/<?= $estimates_info->estimates_id ?>" title="<?= lang('mark_as_cancelled') ?>"><?= lang('mark_as_cancelled') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status == 'cancelled') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/draft/<?= $estimates_info->estimates_id ?>" title="<?= lang('unmark_as_cancelled') ?>"><?= lang('unmark_as_cancelled') ?></a>
                        </li>
                    <?php } ?>
                    <li>
                    ...
                    <?php if (!empty($can_edit) && !empty($edited)) { ?>
                        <li class="divider"></li>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/create/edit_estimates/<?= $estimates_info->estimates_id ?>"><?= lang('edit_estimate') ?></a>
                        </li>
                    <?php } ?>
        ...
        ?>
                <a href="<?= base_url() ?>admin/invoice/manage_invoice/invoice_details/<?= $estimates_info->invoices_id ?>" class="btn btn-xs btn-purple">
        ...
        ?>
        <a class="btn btn-xs btn-green" data-toggle="modal" data-target="#myModal_lg" href="<?= base_url() ?>admin/invoice/reminder/estimate/<?= $estimates_info->estimates_id ?>"><?= lang('reminder') ?>
        ...
        ?>
            ...
            <a href="<?= base_url() ?>admin/projects/project_details/<?= $estimates_info->project_id ?>" class="">
                <?= $project_info->project_name ?>
            </a>
        <?php } ?>
    </div>
    <div class="col-sm-4 pull-right">
        <a href="<?= base_url() ?>admin/estimates/send_estimates_email/<?= $estimates_info->estimates_id . '/' . true ?>" data-toggle="tooltip" data-placement="top" title="<?= lang('send_email') ?>" class="btn btn-xs btn-primary pull-right">
            <i class="fa fa-envelope-o"></i>
        </a>
        <a onclick="print_estimates('print_estimates')" href="#" data-toggle="tooltip" data-placement="top" title="" data-original-title="Print" class="mr-sm btn btn-xs btn-danger pull-right">
            <i class="fa fa-print"></i>
        </a>

        <a href="<?= base_url() ?>admin/estimates/pdf_estimates/<?= $estimates_info->estimates_id ?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="PDF" class="btn btn-xs btn-success pull-right mr-sm">
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-xs-12 ">
                ...
                <div class="pull-right pr-lg">
                    ...
                    ?>
                    <br><?= $language_info['estimate_status'] ?>: <span class="label label-<?= $label ?>"><?= lang($estimates_info->status) ?></span>
