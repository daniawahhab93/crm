<?php class MY_Model extends CI_Model
{
    ...
    {
        //warehouse Qty calculation 
        // $this->invoice_model->_table_name = 'tbl_warehouses_products';
        // $this->invoice_model->_primary_key = 'id';
        if (empty($warehouse_id)) {
            $warehouse_id = $this->session->userdata('warehouse_id');
            if (empty($warehouse_id)) {
                $warehouse_id = $this->db->get('tbl_warehouses')->row()->warehouse_id;
            }
        }
        $checkproduct = get_row('tbl_warehouses_products', array('warehouse_id ' => $warehouse_id, 'product_id' => $id));

        // $_data['quantity'] = $checkproduct->quantity - $qty;
        // $_data_id = $checkproduct->id;
        // $this->invoice_model->save($_data, $_data_id);
    ...
    public function getItemsInfo($term = null, $warehouse_id, $limit = 20)
    {
        ...
        if (!empty($term)) {
            $this->db->where("(item_name LIKE '%" . $term . "%' OR item_desc LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(item_name, ' (', code, ')') LIKE '%" . $term . "%')");
        }
        if (!empty($warehouse_id)) {
            $this->db->where('tbl_warehouses_products.warehouse_id', $warehouse_id);
...
//            }
    ...
    public function getItemsInfoByName($warehouse_id, $limit = 20,$item_code=null,$item_group=null,$item_desc=null )
    {
        ...
        if (!empty($item_code)) {
            $this->db->where("(item_name LIKE '%" . $item_code . "%' )");
        }

        if (!empty($item_desc)) {
            $this->db->where("(item_desc LIKE '%" . $item_desc . "%')");

        }

        if (!empty($item_group)) {
            $this->db->join('tbl_customer_group', 'tbl_customer_group.customer_group_id=tbl_saved_items.customer_group_id');
            $this->db->where("(lower(customer_group) LIKE '%" . strtolower($item_group ) . "%')");
        }
        if (!empty($warehouse_id) && (!empty($item_code) || !empty($item_desc) ||  !empty($item_group) ) ){
            $this->db->where('tbl_warehouses_products.warehouse_id', $warehouse_id);
//            if ($for_purcahse != 'purchase_item') {
//                $this->db->where('tbl_warehouses_products.quantity >', 0);
//            }
...
//        $this->db->group_by('saved_items_id');
        $q = $this->db->get('tbl_saved_items');

        if ($q->num_rows() > 0 &&  (!empty($item_code) || !empty($item_desc) ||  !empty($item_group) )) {
            return $q->result();
        }
        return FALSE;
    }

    public function getItemsInfoByDesc($term = null, $warehouse_id, $limit = 20)
    {
        $for_purcahse = $this->input->get('for', true);
        $this->db->select('tbl_saved_items.*, tbl_warehouses_products.quantity as total_qty');
        $this->db->join('tbl_warehouses_products', 'tbl_warehouses_products.product_id = tbl_saved_items.saved_items_id');
        ...
        if (!empty($term)) {
            $this->db->where("(item_desc LIKE '%" . $term . "%' )");
        }
...
//            }
    ...
    public function getCustomerGroupInfo($term = null,$limit = 20)
    {
      $this->db->select('tbl_customer_group.*');
        if (!empty($term)) {
            $this->db->where("(lower(customer_group) LIKE '%" . strtolower($term ). "%' )");
        }
        $this->db->limit($limit);
        $q = $this->db->get('tbl_customer_group');
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return FALSE;
