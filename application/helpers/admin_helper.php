        ...
        foreach ($all_field as $v_fileds) {
            if (!empty($v_fileds->visible_for_admin) && empty(client())) {
                if (!empty(admin())) {
                    ...
                    if($v_fileds->field_label='item_location_in_stock')
                        $v_fileds->field_label=lang('item_location_in_stock');
...
function supplier_name($supplier_id = null)
{
    $CI = &get_instance();
    if (empty($supplier_id)) {
        $supplier_id = $CI->session->userdata('supplier_id');
    }
    if (is_numeric($supplier_id)) {
        $supplierInfo = $CI->db->where('supplier_id', $supplier_id)->get('tbl_suppliers')->row();
    }
...
function supplier_mobile($supplier_id = null)
{
    $CI = &get_instance();
    if (empty($supplier_id)) {
        $supplier_id = $CI->session->userdata('supplier_id');
    ...
    if (!empty($supplierInfo)) {
        return $supplierInfo->mobile;
    } else {
        return '-';
    }
}

function client_mobile($client_id = null)
{
    $CI = &get_instance();
    if (empty($client_id)) {
        $client_id = $CI->session->userdata('client_id');
...
{
    if ($item->quantity <= $item->quantity_to_change_price) {
        $item->unit_cost = ($item->price_increase_rate * $item->unit_cost / 100) + $item->unit_cost;
    }
    return $item;
}


function get_data_for_search_item()
{
    $saved_items_id = 'all';
    $customer_group_id = 'all';
    $CI =& get_instance();
    if ($CI->input->post()) {
        $saved_items_id = $CI->input->post('saved_items_id', true);
        $customer_group_id = $CI->input->post('customer_group_id', true);
    }
