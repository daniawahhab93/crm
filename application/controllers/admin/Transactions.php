...
class Transactions extends Admin_Controller
{
    ...
    public function import($type)
    {
        ...
        $data['permission_user'] = $this->transactions_model->all_permission_user('30');
        $data['warehouseList'] = $this->transactions_model->select_data('tbl_warehouse', 'warehouse_id', 'warehouse_name', array('status' => 'published'));
        $data['type'] = $type;
        ...
        if (!empty($file)) {
            ...
            if (!empty($valid)) {
                ...
                for ($x = 2; $x <= count($sheetData); $x++) {
                    ...
                    $data = $this->transactions_model->array_from_post(array('name', 'account_id', 'type','warehouse_id', 'category_id', 'paid_by', 'payment_methods_id'));
    ...
    public function save_expense($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            $data = $this->transactions_model->array_from_post(array(
                'transaction_prefix', 'name', 'date', 'notes', 'category_id','warehouse_id', 'paid_by', 'tags', 'payment_methods_id', 'reference', 'project_id', 'billable', 'client_visible',
