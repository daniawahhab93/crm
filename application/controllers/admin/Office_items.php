...
class Office_items extends Admin_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('office_items_model');
    }
    ...
    public function items_list($id = NULL, $opt = null)
    {
        ...
        if (!empty($id)) {
            if (is_numeric($id)) {
                $data['active'] = 2;
                $data['items_info'] = $this->office_items_model->check_by(array('items_id' => $id), 'tbl_office_items');
            } else {
                if ($id == 'manufacturer') {
                    $data['active'] = 4;
                    $data['manufacturer_info'] = $this->office_items_model->check_by(array('manufacturer_id' => $opt), 'tbl_manufacturer');
                } else {
                    $data['active'] = 3;
                    $data['group_info'] = $this->office_items_model->check_by(array('customer_group_id' => $opt), 'tbl_customer_group');
                }
            }
        } else {
            $data['active'] = 1;
        }
        $data['warehouseList'] = $this->office_items_model->select_data('tbl_warehouse', 'warehouse_id', 'warehouse_name', array('status' => 'published'));
        $data['all_customer_group'] = $this->office_items_model->select_data('tbl_customer_group', 'customer_group_id', 'customer_group', array('type' => 'office_items'));
        $data['all_manufacturer'] = $this->office_items_model->select_data('tbl_manufacturer', 'manufacturer_id', 'manufacturer');
        $data['dropzone'] = 1;
        $data['subview'] = $this->load->view('admin/office_items/items_list', $data, TRUE);
    ...
    public function new_items($id = NULL, $opt = null)
    {
        ...
        $data['all_manufacturer'] = $this->office_items_model->select_data('tbl_manufacturer', 'manufacturer_id', 'manufacturer');
        $data['dropzone'] = 1;
        $data['subview'] = $this->load->view('admin/office_items/createitems', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function newitems_group($id = NULL, $opt = null)
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/grouplist', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function itemsList($group_id = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->table = 'tbl_office_items';
...
//            $this->datatables->join_where = array('tbl_customer_group.customer_group_id=tbl_saved_items.customer_group_id', 'tbl_manufacturer.manufacturer_id=tbl_saved_items.manufacturer_id');
            ...
            $main_column = array('item_name', 'quantity', 'unit_cost', 'unit_type', 'tbl_customer_group.customer_group', 'tbl_manufacturer.manufacturer');
            ...
            $this->datatables->order = array('items_id' => 'desc');
            ...
            $data = array();
            $edited = can_action('220', 'edited');
            $deleted = can_action('220', 'deleted');
            foreach ($fetch_data as $_key => $v_items) {
                ...
                $sub_array[] = '<a data-toggle="modal" data-target="#myModal_extra_lg" href="' . base_url('admin/office_items/items_details/' . $v_items->items_id) . '"><strong class="block">' . $item_name . '</strong></a>' . '</span>';

                if (!empty(admin())) {
                    $sub_array[] = display_money($v_items->cost_price, default_currency());
                }
                $sub_array[] = $v_items->date_saved;
...
//                $sub_array[] = display_money($v_items->unit_cost, default_currency());
                ...
                if (!empty($edited)) {
                    $action .= btn_edit('admin/office_items/new_items/' . $v_items->items_id) . ' ';
                }
                if (!empty($deleted)) {
                    $action .= ajax_anchor(base_url("admin/office_items/delete_items/$v_items->items_id"), "<i class='btn btn-xs btn-danger fa fa-trash-o'></i>", array("class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#table_" . $_key)) . ' ';
                }
//                if (!empty($edited)) {
//                    $action .= '<a class="btn btn-inverse btn-xs" data-toggle="tooltip" data-placement="top" title="' . lang('print_barcode') . '"  target="_blank" href="' . base_url('admin/office_items/single_barcode/' . $v_items->saved_items_id) . '"><i class="fa fa-barcode" ></i></a>' . ' ';
//                }
                $sub_array[] = $action;
                $data[] = $sub_array;
            }
    ...
    {
        $data = $this->office_items_model->array_from_post(array('item_name', 'item_desc', 'cost_price', 'quantity','notes'));

        // update root category
        $where = array('item_name' => $data['item_name']);
        // duplicate value check in DB
        if (!empty($id)) { // if id exist in db update data
            $items_id = array('items_id !=' => $id);
        } else { // if id is not exist then set id as null
            $items_id = null;
        }
        // check whether this input data already exist or not
        $check_items = $this->office_items_model->check_update('tbl_office_items', $where, $items_id);
...
//            $data['total_cost'] = $sub_total + $data['item_tax_total'];
            ...
            $this->office_items_model->_table_name = 'tbl_office_items';
            $this->office_items_model->_primary_key = 'items_id';
            $return_id = $this->office_items_model->save($data, $id);
            ...
            if ($warehouse_id && $warehouse_id != 'all') {
                ...
                $this->office_items_model->_table_name = 'tbl_warehouses_products';
                $this->office_items_model->_primary_key = 'id';
                if (!empty($check)) {
                    $_data['quantity'] = $_data['quantity'] + $check->quantity;
                    $_data['product_id'] = $return_id;
                    $_data_id = $check->id;
                    $this->office_items_model->save($_data, $_data_id);
                } else {
                    $this->office_items_model->save($_data);
                }
            } else {
                $warehouses = $this->db->where('status', 'published')->get('tbl_warehouse')->result();


                foreach ($warehouses as $warehouse) {

                    $check = get_row('tbl_warehouses_products', array('warehouse_id ' => $warehouse->warehouse_id, 'product_id' => $_data['product_id']));
                    $_data['warehouse_id'] =  $warehouse->warehouse_id;
            ...
            $this->office_items_model->_table_name = 'tbl_activities';
            $this->office_items_model->_primary_key = 'activities_id';
            $this->office_items_model->save($activity);
            // messages for user
            $type = "success";
        }
        $message = $msg;
        set_message($type, $message);
        redirect('admin/office_items/items_list');
    }
    ...
    public function delete_items($id, $bulk = null)
    {
        $deleted = can_action('220', 'deleted');
        if (!empty($deleted)) {
            $items_info = $this->office_items_model->check_by(array('items_id' => $id), 'tbl_office_items');
            $activity = array(
                'user' => $this->session->userdata('user_id'),
                'module' => 'office_items',
                'module_field_id' => $id,
            ...
            $this->office_items_model->_table_name = 'tbl_office_items';
            $this->office_items_model->_primary_key = 'items_id';
            $this->office_items_model->delete($id);
            $type = 'success';
    ...
    public function items_group()
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/items_group', $data, FALSE);
        $this->load->view('admin/_layout_modal', $data);
    }

    public function update_group($id = null)
    {
        $this->office_items_model->_table_name = 'tbl_customer_group';
        $this->office_items_model->_primary_key = 'customer_group_id';

        $cate_data['customer_group'] = $this->input->post('customer_group', TRUE);
        $cate_data['description'] = $this->input->post('description', TRUE);
        $cate_data['type'] = 'office_items';
        $id = $this->office_items_model->save($cate_data, $id);
    ...
    public function saved_group($id = null)
    {
        ...
        $message = $msg;
        set_message($type, $message);
        redirect('admin/office_items/newitems_group');
    }
    ...
    public function items_manufacturerlist($id = null, $opt = null)
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/manufactureerlist', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }

    public function delete_group($id)
    {
        $customer_group = $this->office_items_model->check_by(array('customer_group_id' => $id), 'tbl_customer_group');
        $activity = array(
            'user' => $this->session->userdata('user_id'),
            'module' => 'settings',
            'module_field_id' => $id,
    ...
    {
        $header = lang('office_items');
        $data['title'] = lang('import_items');
        $data['permission_user'] = $this->office_items_model->all_permission_user('30');
        $data['type'] = 'office_items';
        $data['subview'] = $this->load->view('admin/office_items/import', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
        ...
        if (!empty($file)) {
            ...
            if (!empty($valid)) {
                ...
                for ($x = 2; $x <= count($sheetData); $x++) {
                    ...
                    $category_code_info = $this->office_items_model->check_by(array('type' => 'office_items', 'customer_group' => $category_code), 'tbl_customer_group');
                    if (!empty($category_code_info)) {
                        $data['customer_group_id'] = $category_code_info->customer_group_id;
                    } else {
                        $category['type'] = 'office_items';
                        $category['customer_group'] = $category_code;
                        $this->office_items_model->_table_name = "tbl_customer_group"; //table name
                        $this->office_items_model->_primary_key = "customer_group_id";
                        $data['customer_group_id'] = $this->office_items_model->save($category);
                    }
                    $all_data[] = $data;
                }
                if (!empty($all_data)) {
    ...
    public function items_details($id = NULL)
    {
        ...
        if (!empty($id)) {
            $data['items_info'] = $this->office_items_model->check_by(array('saved_items_id' => $id), 'tbl_saved_items');
            $data['warehouse_info'] = $this->office_items_model->warehouse_porduct($id);
            $data['staff_warehouse_info'] = $this->office_items_model->warehouse_porduct($id, $staffwarehouse);

            $data['barcode'] = $this->product_barcode($data['items_info']->code, $data['items_info']->barcode_symbology, 60);
        }


        $data['subview'] = $this->load->view('admin/office_items/items_details', $data, false);
    ...
    public function single_barcode($id)
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/single_barcode', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }

    function product_barcode($product_code = NULL, $bcs = 'code128', $height = 60)
    {
        return "<img src='" . site_url('admin/office_items/gen_barcode/' . $product_code . '/' . $bcs . '/' . $height) . "' alt='{$product_code}' class='bcimg' />";
    }
    ...
    public function transferItem()
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/transferItem/manage', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function transferItemList()
    {
        if (!empty($this->input->is_ajax_request())) {
            ...
            foreach ($fetch_data as $_key => $v_items) {
                $action = null;
                $can_edit = $this->office_items_model->can_action('tbl_transfer_itemlist', 'edit', array('transfer_itemList_id' => $v_items->transfer_itemList_id));
                $can_delete = $this->office_items_model->can_action('tbl_transfer_itemlist', 'delete', array('transfer_itemList_id' => $v_items->transfer_itemList_id));
                ...
                //                $sub_array[] = '<a data-toggle="modal" data-target="#myModal_extra_lg" href="' . base_url('admin/office_items/transfer_details/' . $v_items->transfer_itemList_id) . '"><strong class="block">' . $item_name .'</strong>';
                $sub_array[] = '<a href="' . base_url() . 'admin/office_items/transfer_details/' . $v_items->transfer_itemList_id . '"><strong class="block">' . $item_name . '</strong></a>';
                ...
                $sub_array[] = display_money($this->office_items_model->transfer_calculate_to('transfer_cost', $v_items->transfer_item_id));
                ...
                if (!empty($can_edit) || !empty($edited)) {
                    $ch_url = base_url() . 'admin/office_items/change_status/';
        ...
        <ul class="dropdown-menu animated zoomIn">
            ...
            <li>
                ...
                if (!empty($can_edit) && !empty($edited)) {
                    $action .= btn_edit('admin/office_items/createTransferItem/' . $v_items->transfer_item_id) . ' ';
                }
                if (!empty($can_delete) && !empty($deleted)) {
                    $action .= ajax_anchor(base_url("admin/office_items/delete_transfer/" . $v_items->transfer_item_id), "<i class='btn btn-xs btn-danger fa fa-trash-o'></i>", array("class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#table_" . $_key)) . ' ';
                }
                if (!empty($can_edit) && !empty($edited)) {
                    $action .= btn_view('admin/office_items/transfer_details/' . $v_items->transfer_itemList_id) . ' ';
                }
                $sub_array[] = $action;
    ...
    public function transfer_details($id = NULL)
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/transferItem/transfer_details', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function createTransferItem($id = null)
    {
        ...
        $data['warehouseList'] = $this->office_items_model->select_data('tbl_warehouse', 'warehouse_id', 'warehouse_name', array('status' => 'published'));
        $data['subview'] = $this->load->view('admin/office_items/transferItem/create', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    ...
    public function saveTransferItem($id = null)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            $data = $this->office_items_model->array_from_post(array('reference_no', 'date', 'to_warehouse_id', 'status', 'notes', 'show_quantity_as'));
            ...
            if ($data['from_warehouse_id'] == $data['to_warehouse_id']) {
                ...
                redirect('admin/office_items/createTransferItem');
            }
            if (!empty($data['to_warehouse_id'])) {
                ...
                if (!empty($permission)) {
                    if ($permission == 'everyone') {
                        ...
                        $assigned_to = $this->office_items_model->array_from_post(array('assigned_to'));
                ...
                $items_data = $this->input->post('office_items', true);
                ...
                $activity = array(
                    ...
                    'link' => 'admin/office_items/transfer_details/' . $transfer_itemList_id,
                ...
                $message = $msg;
                set_message($type, $message);
                redirect('admin/office_items/transferItem');
            } else {
                redirect('admin/office_items/createTransferItem');
            }
        } else {
    ...
    {
        $saved_items = $this->office_items_model->select_group_data('tbl_saved_items', 'customer_group', 'saved_items_id', 'saved_items_id,item_name,item_desc,unit_cost', ['tbl_customer_group.type' => 'office_items', 'tbl_saved_items.warehouse_id' => $warehouse_id], ['tbl_customer_group' => 'tbl_customer_group.customer_group_id = tbl_saved_items.customer_group_id'], true);
    ...
    public function manuallyItems()
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/manually_items', $data, false);
        $this->load->view('admin/_layout_modal', $data);
    }

    public function saved_manufacturer($id = null)
    {
        $data = $this->office_items_model->array_from_post(array('manufacturer', 'description'));
        $this->office_items_model->_table_name = "tbl_manufacturer"; //table name
        $this->office_items_model->_primary_key = "manufacturer_id";
        $id = $this->office_items_model->save($data, $id);
        // messages for user
        $type = "success";
        if (!empty($id)) {
        ...
        set_message($type, $message);
        redirect('admin/office_items/items_manufacturerlist');
    }
    ...
    public function TransferItemDetails($id = null)
    {
        ...
        $data['dropzone'] = true;
        $data['warehouseList'] = $this->office_items_model->select_data('tbl_warehouse', 'warehouse_id', 'warehouse_name', array('status' => 'published'));
        $data['subview'] = $this->load->view('admin/office_items/transferItem/TransferItemDetails', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    ...
    function clone_transfer_item($item_id)
    {
        ...
        $can_edit = $this->office_items_model->can_action('tbl_transfer_itemlist', 'edit', array('transfer_itemList_id' => $item_id));

        if (!empty($can_edit) && !empty($edited)) {

            $data['item_info'] = $this->office_items_model->check_by(array('transfer_itemList_id' => $item_id), 'tbl_transfer_itemlist');
            $data['all_warehouse'] = $this->office_items_model->get_permission('tbl_warehouse');


            $data['modal_subview'] = $this->load->view('admin/office_items/transferItem/_modal_clone_transferitem', $data, FALSE);
        } else {
            set_message('error', lang('there_in_no_value'));
            redirect($_SERVER['HTTP_REFERER']);
        }
    ...
    public function manufacturer()
    {
        ...
        $data['subview'] = $this->load->view('admin/office_items/manufacturer', $data, FALSE);
        $this->load->view('admin/_layout_modal', $data);
    }

    public function update_manufacturer($id = null)
    {
        $this->office_items_model->_table_name = 'tbl_manufacturer';
        $this->office_items_model->_primary_key = 'manufacturer_id';
        ...
        // check whether this input data already exist or not
        $check_category = $this->office_items_model->check_update('tbl_manufacturer', $where, $manufacturer_id);
    ...
    public function pdf_transferitem($id)
    {
        ...
        $viewfile = $this->load->view('admin/office_items/transferItem/transfer_pdf', $data, TRUE);
    ...
    public function change_status($id, $status)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            $item_info = $this->office_items_model->check_by(array('transfer_item_id' => $id), 'tbl_transfer_item');
            ...
            $data['status'] = $status;
            $this->office_items_model->_table_name = 'tbl_transfer_item';
            $this->office_items_model->_primary_key = 'transfer_item_id';
            $lastid = $this->office_items_model->save($data, $id);
            // messages for user
            $type = "success";
    ...
    public function UpdateProductQty($product_list, $to_warehouse_id, $from_warehouse_id)
    {
        ...
        foreach ($product_list as $key) {
            ...
            if (!empty($check_already_exist_by_to_id)) {
                ...
                $this->office_items_model->save($_data2, $data2_id);
            } else {
                ...
                $this->office_items_model->save($data, $_data_id);
            }
        }
    }
    ...
    public function delete_transfer($id)
    {
        ...
        if (!empty($deleted)) {
            $items_info = $this->office_items_model->check_by(array('transfer_item_id' => $id), 'tbl_transfer_item');
            $activity = array(
                'user' => $this->session->userdata('user_id'),
                'module' => 'office_items',
                'module_field_id' => $id,
            ...
            if (!empty($transfer_info)) {
                ...
                foreach ($transfer_info as $t_v_files) {
                    $this->office_items_model->_table_name = "tbl_transfer_itemlist"; //table name
                    $this->office_items_model->delete_multiple(array('transfer_item_id' => $t_v_files->transfer_item_id));
                }
            }
