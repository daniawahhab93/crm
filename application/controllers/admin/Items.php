...
class Items extends Admin_Controller
{
    ...
    {
        $saved_items = $this->db->get('tbl_saved_items')->result();
        foreach ($saved_items as $item) {
            $data_p = array(
                array(
                    'product_id' => $item->saved_items_id,
                    'warehouse_id' => 1,
                ),
                array(
                    'product_id' => $item->saved_items_id,
            ...
            $this->db->insert_batch('tbl_warehouses_products', $data_p);
    ...
    public function new_items($id = NULL, $opt = null)
    {
        ...
        $data['warehouseList'] = $this->items_model->select_data('tbl_warehouse', 'warehouse_id', 'warehouse_name', array('status' => 'published'));
        $data['alternativeItems'] = $this->db->select('saved_items_id,item_name,item_desc')->get('tbl_saved_items')->result();
        $data['all_customer_group'] = $this->items_model->select_data('tbl_customer_group', 'customer_group_id', 'customer_group', array('type' => 'items'));
        $data['all_manufacturer'] = $this->items_model->select_data('tbl_manufacturer', 'manufacturer_id', 'manufacturer');
        $data['dropzone'] = 1;
    ...
    {
        $data = $this->items_model->array_from_post(array('item_name', 'alternative_items', 'old_code', 'minimum_in_stock', 'manufacturer_id', 'code', 'barcode_symbology', 'item_desc', 'hsn_code', 'cost_price', 'unit_cost', 'unit_type', 'customer_group_id', 'quantity', 'item_location_in_stock'));
        ...
        if (!empty($check_items)) { // if input data already exist show error alert
            ...
            $alternative_items = $this->input->post("alternative_items", true);
            if ($alternative_items) {
                $data['alternative_items'] = json_encode($alternative_items);
            }
    ...
    public function saveTransferItem($id = null)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            $data['source_type'] = $this->input->post('source_type', true);
            $data['transfer_type'] = $this->input->post('transfer_type', true);
