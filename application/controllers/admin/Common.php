...
class Common extends MY_Controller
{
    ...
    {
        $attachment_info = $this->db->where($type, $id)->get('tbl_attachments')->result();
        if ($type == 'project_id') {
...
//            $row->rate = $row->unit_cost;
            ...
            $pr = array('saved_items_id' => str_replace(".", "", microtime(true)), 'saved_items_id' => $row->saved_items_id, 'label' => $row->item_name, 'row' => $result);
            echo json_encode($pr);
            die();
        }
        ...
        if (!empty($rows)) {
            foreach ($rows as $row) {
                if ($client_id) {
                    ...
                    $row->lowest_sale_price_item = '-';
                    $last_item = $this->invoice_model->get_last_sale_price_item($row->saved_items_id, $client_id);
                    $lowest_item = $this->invoice_model->get_lowest_sale_price_item($row->saved_items_id, $client_id);
                    if ($last_item && $last_item->unit_cost) {
                        $row->last_sale_price_item = $last_item->unit_cost;
                        $row->lowest_sale_price_item = $lowest_item->unit_cost;
                    }
                }

                if ($supplier_id) {
                    $row->last_purchase_price_same_supplier = '-';
                    $row->lowest_purchase_price = '-';
                    $row->last_cost_price = '-';
                    $row->lowest_cost_price = '-';

                    $last_purchase_price_same_supplier = $this->purchase_model->get_last_purchase_price_same_supplier_items($row->saved_items_id, $supplier_id);
                    if ($last_purchase_price_same_supplier && $last_purchase_price_same_supplier->unit_cost)
                        $row->last_purchase_price_same_supplier = $last_purchase_price_same_supplier->unit_cost;

                    $lowest_purchase_price = $this->purchase_model->get_lowest_purchase_price_items($row->saved_items_id);
                    if ($lowest_purchase_price && $lowest_purchase_price->unit_cost)
                        $row->lowest_purchase_price = $lowest_purchase_price->unit_cost;

                    $last_cost_price = $this->purchase_model->get_last_cost_price_items($row->saved_items_id);
                    if ($last_cost_price && $last_cost_price->cost_price)
                        $row->last_cost_price = $last_cost_price->cost_price;


                    $lowest_cost_price = $this->purchase_model->get_lowest_cost_price_items($row->saved_items_id);
                    if ($lowest_cost_price && $lowest_cost_price->cost_price)
                        $row->lowest_cost_price = $lowest_cost_price->cost_price;
                }
                ...
                $alternative_items = '';
                if ($row->alternative_items) {
                    $alt = json_decode($row->alternative_items, true);
                    foreach ($alt as $a_i) {
                        $info = $this->items_model->check_by(array('saved_items_id' => $a_i), 'tbl_saved_items');
                        $alternative_items .= '<a  data-toggle="modal" data-target="#myModal_extra_lg" href="' . base_url("admin/items/items_details/" . $a_i) . '">' . $info->item_name . '</a><br>';

                    }
                }
                $row->alternative_items = $alternative_items;
                ...
                $result = (object)array_merge((array)$row, (array)$tax);
                $pr[] = array('saved_items_id' => $row->saved_items_id, 'label' => $row->item_name, 'row' => $result);
            }
            echo json_encode($pr);
            die();
        } else {
    ...
    function suggestionsName($id = NULL)
    {
        $client_id = $this->input->get('client_id', TRUE);
        $supplier_id = $this->input->get('supplier_id', TRUE);
        $item_code = $this->input->get('item_code', TRUE);
        $item_group = $this->input->get('item_group', TRUE);
        ...
        if (!empty($rows)) {
            foreach ($rows as $key => $row) {
                ...
                $i=$key+1;
                $row->tax_dropdown =$this->admin_model->get_taxes_dropdown('items['.$i.'][taxname][]', $tax);
                $result = (object)array_merge((array)$row, (array)$tax);
                $pr[] = array('saved_items_id' => $row->saved_items_id, 'label' => $row->item_name, 'row' => $result);
            }
            echo json_encode($pr);
    ...
    function suggestionsDesc($id = NULL)
    {
        $client_id = $this->input->get('client_id', TRUE);
        $supplier_id = $this->input->get('supplier_id', TRUE);
        if (!empty($id)) {
...
//            $row->rate = $row->unit_cost;
    ...
    public function suggestionsGroups($warehouseId = NULL)
    {
        $term = $this->input->get('term', TRUE);
        $warehouse_id = $this->input->get('warehouse_id', TRUE);

        $rows = $this->admin_model->getCustomerGroupInfo($term);
    ...
    public function save_comments_reply($comment_id)
    {
        ...
        $comment_id = $this->common_model->save($data);
        if (!empty($comment_id)) {
            $comments_info = $this->common_model->get_comment_details($comment_id);
