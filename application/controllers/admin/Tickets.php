...
class Tickets extends Admin_Controller
{
    ...
    final public function create($action = NULL, $id = NULL)
    {
        ...
        if ($action == 'tickets_details') {
            ...
            $activities = array(
                ...
                'icon' => 'fa-ticket',
                'link' => 'admin/tickets/create/tickets_details/' . $id,
                'value1' => $rdata['body'],
            );
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_tickets_info) {
                if (!empty($v_tickets_info)) {
                    ...
                    $ticket_code = null;
                    $ticket_code .= '<a class="text-info" href="' . base_url() . 'admin/tickets/create/tickets_details/' . $v_tickets_info->tickets_id . '">' . $v_tickets_info->ticket_code . '</a>';
                    $sub_array[] = $ticket_code;

                    $ticket_subject = null;
                    $ticket_subject .= '<a class="text-info" href="' . base_url() . 'admin/tickets/create/tickets_details/' . $v_tickets_info->tickets_id . '">' . $v_tickets_info->subject . '</a>';
    ...
    <img style="width: 30px;margin-left: 18px;height: 29px;border: 1px solid #aaa;"
         ...
         class="img-circle"></a>';
                    ...
                    $action .= btn_view('admin/tickets/create/tickets_details/' . $v_tickets_info->tickets_id) . ' ';
    ...
    public function create_tickets($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            if (!empty($data['project_id']) && is_numeric($data['project_id'])) {
                ...
                redirect('admin/tickets/create/tickets_details/' . $id);
            }
        } else {
            if (empty($_SERVER['HTTP_REFERER'])) {
                redirect('admin/tickets');
    ...
    function notify_ticket_reopened($id)
    {
        ...
        $TicketLink = str_replace("{TICKET_LINK}", base_url() . 'admin/tickets/create/tickets_details/' . $ticket_info->tickets_id, $USER);
                ...
                if (!empty($v_user)) {
                    ...
                    if ($v_user->user_id != $this->session->userdata('user_id')) {
                        add_notification(array(
                            ...
                            'link' => 'admin/tickets/create/tickets_details/' . $ticket_info->tickets_id,
    ...
    public function save_comments_reply($tickets_replies_id)
    {
        ...
        if (!empty($tickets_replies_id)) {
            ...
            $activities = array(
                ...
                'activity' => 'activity_reply_tickets',
                'icon' => 'fa-ticket',
                'link' => 'admin/tickets/create/tickets_details/' . $rdata['tickets_id'],
                'value1' => $rdata['body'],
            );
            // Update into tbl_project
            $this->tickets_model->_table_name = "tbl_activities"; //table name
    ...
    function send_tickets_info_by_email($postdata, $client = NULL)
    {
        ...
        if (!empty($client)) {
            ...
            $TicketLink = str_replace("{TICKET_LINK}", base_url() . 'admin/tickets/create/tickets_details/' . $ticket_info->tickets_id, $ReporterEmail);
            $message = str_replace("{SITE_NAME}", config_item('company_name'), $TicketLink);
            $data['message'] = $message;
            $message = $this->load->view('email_template', $data, TRUE);
                    ...
                    foreach ($user_info as $v_user) {
                        ...
                        if ($v_user->user_id != $this->session->userdata('user_id')) {
                            add_notification(array(
                                ...
                                'link' => 'admin/tickets/create/tickets_details/' . $tickets_info->tickets_id,
